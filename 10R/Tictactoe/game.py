...
import numpy as np
from config import _V, _H, _Gravity
...
class GameState():
    def __init__(self,boardPieces,original=None):
        ...
        self.wList=[]
        for i in range(_V):
            self.wList.append(list(zip([i]*_H,range(_H))))
        for j in range(_H):
            self.wList.append(list(zip(range(_V),[j]*_V)))
        self.wList.append(list(zip(range(_V),range(_H))))
        self.wList.append(list(zip(range(_V),reversed(range(_H)))))
    ...
    def checkEnd(self):
        ...
        for w in self.wList:
            ...
            for i,j in w:
                if self.boardPieces[i,j]!=self.bool2sign(not self.currentPlayer):
                    end=False
            if end==True:
                self.isEnd=True
